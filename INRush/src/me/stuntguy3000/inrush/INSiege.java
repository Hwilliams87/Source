...
public class INSiege extends JavaPlugin {
		...
		Bukkit.getScheduler().runTaskTimer(this, new Runnable() {
			...
			public void run() {
				...
				if (gs.getGameState() == Gamestate.WAITING || gs.getGameState() == Gamestate.WARMUP)
					status = serverID + "," + Bukkit.getOnlinePlayers().length + "," + Bukkit.getMaxPlayers() + ",LOBBY";
				else status = serverID + "," + Bukkit.getOnlinePlayers().length + "," + Bukkit.getMaxPlayers() + "," + gs.getGameState();
	...
	private void initGameSettings() {
		...
		gs = new GameSettings(Gamestate.WAITING, 90, maps.get(new Random().nextInt(maps.size())), 0, 0, 50);
	}
